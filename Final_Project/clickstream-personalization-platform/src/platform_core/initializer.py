...
EXPECTED_AUDIT_TABLES = (
    "pipeline_runs", "quality_metrics", "quarantine_records", "external_api_failures",
    "watermarks", "validation_runs", "serving_builds",
)
...
def _safe_error(error: Exception | str, *, limit: int = 500) -> str:
    ...
    return message[-limit:] if message else error.__class__.__name__ if isinstance(error, Exception) else "Unknown error"
...
def _write_init_report(project_root: Path, results: list[InitCheckResult], passed: bool, *, run_id: str | None = None) -> None:
...
def _source_manifest(project_root: Path) -> dict[str, Any]:
    ...
    if not manifest_path.exists():
        raise FileNotFoundError("Missing data/source/generation_manifest.json. Run: python main.py init")
...
def preflight(project_root: Path) -> tuple[list[InitCheckResult], bool]:
    ...
    if infra_ok:
        ...
        results.append(_result("FAIL", "infrastructure infrastructure", f"Fix before init: {', '.join(failed[:5])}"))

    source_results, source_ok = validate_sources(project_root, write_report=True)
    if source_ok:
        results.append(_result("PASS", "Source generation local sources", "Source contracts, lineage, and manifest checks passed"))
    else:
        failed = [item.check for item in source_results if item.status == "FAIL"]
        results.append(_result("FAIL", "Source generation local sources", f"Fix before init: {', '.join(failed[:5])}"))
    ...
    try:
        ...
        if product_count > 0 and len(checksum) == 64:
            results.append(_result("PASS", "Static catalog manifest", f"{product_count} products; checksum={checksum[:12]}"))
        else:
            results.append(_result("FAIL", "Static catalog manifest", "Missing product count or SHA-256 checksum"))
    ...
    try:
        ...
        if configured_bucket == expected_bucket:
            results.append(_result("PASS", "MinIO bucket contract", f"{configured_bucket} matches the Iceberg warehouse"))
        else:
            results.append(
                _result(
                    "FAIL",
...
def ensure_kafka_topics(project_root: Path) -> InitCheckResult:
    ...
    try:
        ...
        admin = AdminClient(
            {
                "bootstrap.servers": settings["runtime"]["host"][
            ...
            futures = admin.create_topics(
                [
                    NewTopic(
                        ...
                        replication_factor=specs[topic_name][
                            "replication_factor"
                        ],
            ...
            for topic_name, future in futures.items():
                try:
                    ...
                    if "TOPIC_ALREADY_EXISTS" not in message:
                        raise RuntimeError(
                            f"Topic {topic_name}: {_safe_error(error)}"
                        ) from error
        ...
        while time.monotonic() < deadline:
            ...
            for topic_name, spec in specs.items():
                ...
                if len(partitions) != spec["partitions"]:
                    failures.append(
                        f"{topic_name}: partitions={len(partitions)}"
                    )
                    continue
                ...
                if replica_lengths != {spec["replication_factor"]}:
                    failures.append(
                        f"{topic_name}: replicas={sorted(replica_lengths)}"
                    )
                ...
                if min(
                    ...
                    failures.append(
                        f"{topic_name}: ISR={sorted(isr_lengths)}"
                    )

            if not failures:
                created_text = ( f"created {len(missing_topics)}" if missing_topics else "all already existed" )

                return _result( "PASS", "Kafka topic contract", ( f"{len(specs)} topics verified; " f"{created_text}; RF=3" ), )

            last_failures = failures
            time.sleep(3)

        return _result( "FAIL", "Kafka topic contract", ( "Timed out while waiting for Kafka topic metadata and " f"replication: {'; '.join(last_failures)}" ), )

    except Exception as error:
        return _result( "FAIL", "Kafka topic contract", _safe_error(error), )
...
def ensure_minio_bucket(project_root: Path) -> InitCheckResult:
    ...
    try:
        ...
        return _result("PASS", "MinIO Iceberg bucket", f"Bucket {bucket} exists and is reachable through S3 API")
    except Exception as error:
...
def ensure_postgres_source_schema(project_root: Path) -> InitCheckResult:
    ...
    try:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if found != set(EXPECTED_POSTGRES_TABLES):
                    raise RuntimeError(f"Expected source tables missing: {sorted(set(EXPECTED_POSTGRES_TABLES) - found)}")
        ...
        return _result("PASS", "PostgreSQL source schema", "users, orders, order_items created and intentionally empty")
    except Exception as error:
...
def _run_spark_job(project_root: Path, script_name: str, run_id: str, *, timeout: int = 420) -> tuple[bool, str]:
...
def bootstrap_iceberg_and_catalog(project_root: Path, *, run_id: str) -> InitCheckResult:
    ...
    try:
        ...
        if not ok:
            return _result("FAIL", "Spark Iceberg bootstrap", _safe_error(detail or "spark-submit failed"))
        if not report_path.exists():
            return _result("FAIL", "Spark Iceberg bootstrap", "Spark completed but did not write its Initialization report")
        report = json.loads(report_path.read_text(encoding="utf-8"))
        if report.get("status") != "PASSED":
            return _result("FAIL", "Spark Iceberg bootstrap", str(report.get("error", "Spark report is not PASSED")))
        return _result(
            "PASS",
...
def _verify_postgres_schema(project_root: Path) -> InitCheckResult:
    ...
    try:
        ...
        connection.close()
        if found == set(EXPECTED_POSTGRES_TABLES):
            return _result("PASS", "PostgreSQL source tables", "users, orders, order_items are ready for the later CDC load")
        return _result("FAIL", "PostgreSQL source tables", f"Missing: {sorted(set(EXPECTED_POSTGRES_TABLES) - found)}")
    except Exception as error:
...
def verify_minio_warehouse(project_root: Path) -> InitCheckResult:
    ...
    try:
        ...
        if count <= 0:
            return _result("FAIL", "MinIO Iceberg objects", "Bucket exists but warehouse/ contains no Iceberg objects")
        return _result("PASS", "MinIO Iceberg objects", f"warehouse/ contains at least {count} Iceberg object(s)")
    except Exception as error:
...
def verify_iceberg_tables(project_root: Path, *, run_id: str) -> InitCheckResult:
    ...
    try:
        ...
        if not ok:
            return _result("FAIL", "Iceberg read verification", _safe_error(detail or "spark-submit failed"))
        if not report_path.exists():
            return _result("FAIL", "Iceberg read verification", "Spark verification completed without its report")
        report = json.loads(report_path.read_text(encoding="utf-8"))
        if report.get("status") != "PASSED":
            return _result("FAIL", "Iceberg read verification", str(report.get("error", "Verification report is not PASSED")))
        return _result(
            "PASS",
...
def verify_initialization(project_root: Path) -> tuple[list[InitCheckResult], bool, str]:
    ...
    for action in (_verify_topics, ensure_minio_bucket, _verify_postgres_schema, verify_minio_warehouse):
        result = action(project_root)
        results.append(result)
