...
GEOIP_PATH = (
    Path(_geoip_value) if _geoip_value else PROJECT_ROOT / "data/reference/GeoLite2-City.mmdb"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the all-source Spark Structured Streaming application"
    )
...
def raw_new_records(spark: SparkSession, batch: DataFrame, batch_id: int) -> DataFrame:
    ...
    raw = batch.select(
        F.element_at(
            F.create_map(
                *sum(
                    ([F.lit(topic), F.lit(source)] for topic, source in TOPIC_TO_SOURCE.items()), []
                )
            ),
            F.col("kafka_topic"),
        ).alias("source_name"),
        ...
        "kafka_offset",
        "kafka_timestamp",
        F.concat_ws("|", "kafka_topic", "kafka_partition", "kafka_offset").alias(
            "source_record_id"
        ),
        "raw_payload",
        F.lit(None).cast("string").alias("source_file"),
        F.current_timestamp().alias("ingested_at"),
        F.lit(batch_id).cast("long").alias("stream_batch_id"),
    )
...
def unprocessed_records(
    spark: SparkSession, records: DataFrame, clean_table: str, source_name: str
) -> DataFrame:
    ...
    quarantine_ids = (
        spark.table(f"{CATALOG}.audit.quarantine_records")
...
def quarantine(df: DataFrame, source_name: str, batch_id: int) -> int:
    rows = df.select(
        F.sha2(F.concat_ws("|", "source_record_id", "reason_code"), 256).alias("quarantine_id"),
...
def deduplicate(
    spark: SparkSession, candidates: DataFrame, key: str, table: str
...
def audit_run(
    ...
    spark.createDataFrame(
        metrics,
        "run_id string, source_name string, metric_name string, metric_value string, status string, recorded_at timestamp",
    ).writeTo(f"{CATALOG}.audit.quality_metrics").append()
...
def payload_for_web_log(raw_payload: F.Column) -> F.Column:
    ...
    return F.when(F.get_json_object(raw_payload, "$.log_id").isNotNull(), raw_payload).otherwise(
        F.coalesce(
            F.get_json_object(raw_payload, "$.message"),
            F.get_json_object(raw_payload, "$.event.original"),
            raw_payload,
        )
    )
...
def process_clickstream(
    spark: SparkSession, records: DataFrame, batch_id: int, run_id: str, allowed_lateness: int
) -> None:
    records = unprocessed_records(
        spark, records, f"{CATALOG}.processed.clickstream_clean", "clickstream"
    )
    ...
    parsed = records.select(
        ...
        "kafka_offset",
        "source_record_id",
        F.get_json_object("raw_payload", "$.contract_version").alias("contract_version"),
        F.get_json_object("raw_payload", "$.event_id").alias("event_id"),
        F.to_timestamp(
            F.get_json_object("raw_payload", "$.event_timestamp"), "yyyy-MM-dd'T'HH:mm:ssX"
        ).alias("event_timestamp"),
        F.get_json_object("raw_payload", "$.session_id").alias("session_id"),
        F.get_json_object("raw_payload", "$.visitor_id").alias("visitor_id"),
        F.get_json_object("raw_payload", "$.request_id").alias("request_id"),
        F.get_json_object("raw_payload", "$.user_id").alias("user_id"),
        F.get_json_object("raw_payload", "$.event_type").alias("event_type"),
        F.get_json_object("raw_payload", "$.page_url").alias("page_url"),
        F.get_json_object("raw_payload", "$.search_query").alias("search_query"),
        F.get_json_object("raw_payload", "$.product_id").alias("product_id"),
        F.get_json_object("raw_payload", "$.checkout_id").alias("checkout_id"),
        F.get_json_object("raw_payload", "$.order_id").alias("order_id"),
        F.get_json_object("raw_payload", "$.ip_address").alias("ip_address"),
        F.get_json_object("raw_payload", "$.device_type").alias("device_type"),
        F.get_json_object("raw_payload", "$.browser").alias("browser"),
        F.get_json_object("raw_payload", "$.operating_system").alias("operating_system"),
        F.get_json_object("raw_payload", "$.traffic_source").alias("traffic_source"),
        F.get_json_object("raw_payload", "$.scroll_depth_pct")
        .cast("int")
        .alias("scroll_depth_pct"),
        F.get_json_object("raw_payload", "$.time_on_page_seconds")
        .cast("int")
        .alias("time_on_page_seconds"),
    )
    invalid_reason = (
        F.when(F.get_json_object("raw_payload", "$").isNull(), F.lit("MALFORMED_JSON"))
        .when(F.col("contract_version") != "1.0", F.lit("UNSUPPORTED_CONTRACT_VERSION"))
        .when(F.col("event_id").isNull() | (F.length("event_id") == 0), F.lit("MISSING_EVENT_ID"))
        .when(F.col("event_timestamp").isNull(), F.lit("INVALID_EVENT_TIMESTAMP"))
        .when(
            F.col("session_id").isNull() | (F.length("session_id") == 0),
            F.lit("MISSING_SESSION_ID"),
        )
        .when(
            F.col("visitor_id").isNull() | (F.length("visitor_id") == 0),
            F.lit("MISSING_VISITOR_ID"),
        )
        .when(~F.col("event_type").isin(*VALID_EVENT_TYPES), F.lit("UNSUPPORTED_EVENT_TYPE"))
        .when(
            F.col("event_type").isin(*PRODUCT_EVENTS)
            & (F.col("product_id").isNull() | (F.length("product_id") == 0)),
            F.lit("MISSING_PRODUCT_ID"),
        )
        .when(
            F.col("event_type").isin(*CHECKOUT_EVENTS)
            & (F.col("checkout_id").isNull() | (F.length("checkout_id") == 0)),
            F.lit("MISSING_CHECKOUT_ID"),
        )
        .when(
            (F.col("event_type") == F.lit("checkout_complete"))
            & (F.col("order_id").isNull() | (F.length("order_id") == 0)),
            F.lit("MISSING_ORDER_ID"),
        )
    )
    invalid = (
        parsed.withColumn("reason_code", invalid_reason)
        .filter(F.col("reason_code").isNotNull())
        .withColumn("reason_description", F.col("reason_code"))
    )
    valid = (
        parsed.withColumn("reason_code", invalid_reason)
        .filter(F.col("reason_code").isNull())
        .drop("reason_code")
    )
    accepted, duplicates = deduplicate(
        spark, valid, "event_id", f"{CATALOG}.processed.clickstream_clean"
    )
    invalid_count = quarantine(invalid, "clickstream", batch_id)
    duplicates = duplicates.withColumn("reason_code", F.lit("DUPLICATE_EVENT_ID")).withColumn(
        "reason_description", F.lit("event_id already exists in Clean data or this micro-batch")
    )
    duplicate_count = quarantine(duplicates, "clickstream", batch_id)
    max_time = (
        accepted.agg(F.max("event_timestamp").alias("max_time")).collect()[0]["max_time"]
        ...
        accepted.withColumn(
            ...
            (
                ...
                if max_time is None
                    ...
                    < F.lit(max_time) - F.expr(f"INTERVAL {allowed_lateness} MINUTES")
                )
            ),
        )
        .withColumn("geo", geo_udf("ip_address"))
        .select(
            "event_id",
            "contract_version",
            "event_timestamp",
            ...
            "late_arrival",
            F.col("geo.country_code").alias("geo_country_code"),
            F.col("geo.country_name").alias("geo_country_name"),
            F.col("geo.city").alias("geo_city"),
            F.col("geo.latitude").alias("geo_latitude"),
            F.col("geo.longitude").alias("geo_longitude"),
            F.col("geo.timezone").alias("geo_timezone"),
            F.lit("clickstream_events.jsonl").alias("source_file"),
            "kafka_topic",
            "kafka_partition",
            "kafka_offset",
            "source_record_id",
...
def process_web_logs(
    ...
    records = unprocessed_records(
        spark, records, f"{CATALOG}.processed.webserver_logs_clean", "web_logs"
    )
    if records.limit(1).count() == 0:
        return
    ...
    parsed = records.select(
        ...
        "kafka_offset",
        "source_record_id",
        F.get_json_object("payload", "$.contract_version").alias("contract_version"),
        F.get_json_object("payload", "$.log_id").alias("log_id"),
        F.get_json_object("payload", "$.request_id").alias("request_id"),
        F.to_timestamp(F.get_json_object("payload", "$.timestamp"), "yyyy-MM-dd'T'HH:mm:ssX").alias(
            "log_timestamp"
        ),
        F.get_json_object("payload", "$.ip_address").alias("ip_address"),
        F.get_json_object("payload", "$.http_method").alias("http_method"),
        F.get_json_object("payload", "$.endpoint").alias("endpoint"),
        F.get_json_object("payload", "$.status_code").cast("int").alias("status_code"),
        F.get_json_object("payload", "$.response_time_ms").cast("int").alias("response_time_ms"),
        F.get_json_object("payload", "$.user_agent").alias("user_agent"),
        F.get_json_object("payload", "$.bytes_sent").cast("long").alias("bytes_sent"),
    )
    invalid_reason = (
        F.when(F.get_json_object("raw_payload", "$").isNull(), F.lit("MALFORMED_JSON"))
        .when(F.col("contract_version") != "1.0", F.lit("UNSUPPORTED_CONTRACT_VERSION"))
        .when(F.col("log_id").isNull() | (F.length("log_id") == 0), F.lit("MISSING_LOG_ID"))
        .when(
            F.col("request_id").isNull() | (F.length("request_id") == 0),
            F.lit("MISSING_REQUEST_ID"),
        )
        .when(F.col("log_timestamp").isNull(), F.lit("INVALID_LOG_TIMESTAMP"))
        .when(F.col("endpoint").isNull() | (F.length("endpoint") == 0), F.lit("MISSING_ENDPOINT"))
        .when(
            F.col("status_code").isNull()
            | (F.col("status_code") < 100)
            | (F.col("status_code") > 599),
            F.lit("INVALID_STATUS_CODE"),
        )
        .when(
            F.col("response_time_ms").isNull() | (F.col("response_time_ms") < 0),
            F.lit("INVALID_RESPONSE_TIME"),
        )
    )
    invalid = (
        parsed.withColumn("reason_code", invalid_reason)
    ...
    accepted, duplicates = deduplicate(
        spark, valid, "log_id", f"{CATALOG}.processed.webserver_logs_clean"
    )
    invalid_count = quarantine(invalid, "web_logs", batch_id)
    duplicates = duplicates.withColumn("reason_code", F.lit("DUPLICATE_LOG_ID")).withColumn(
        "reason_description", F.lit("log_id already exists in Clean data or this micro-batch")
    )
    duplicate_count = quarantine(duplicates, "web_logs", batch_id)
    max_time = (
        accepted.agg(F.max("log_timestamp").alias("max_time")).collect()[0]["max_time"]
        if accepted.limit(1).count()
        else None
    )
    ready = (
        ...
        .select(
            ...
            F.col("geo.longitude").alias("geo_longitude"),
            F.col("geo.timezone").alias("geo_timezone"),
            F.lit("webserver_access.log").alias("source_file"),
            "kafka_topic",
            "kafka_partition",
            "kafka_offset",
            "source_record_id",
...
def process_cdc(
    ...
    parsed = (
        ...
        .select(
            ...
            "kafka_offset",
            "source_record_id",
            F.get_json_object("payload", "$.op").alias("operation"),
            F.get_json_object("payload", "$.before").alias("before_json"),
            F.get_json_object("payload", "$.after").alias("after_json"),
            F.coalesce(
                F.get_json_object("payload", "$.source.lsn").cast("long"), F.col("kafka_offset")
            ).alias("source_lsn"),
            F.get_json_object("payload", "$.ts_ms").cast("long").alias("source_ts_ms"),
            F.get_json_object("payload", f"$.after.{key}").alias("after_key"),
            F.get_json_object("payload", f"$.before.{key}").alias("before_key"),
            F.get_json_object("payload", "$.after.user_id").alias("user_id_after"),
            F.get_json_object("payload", "$.before.user_id").alias("user_id_before"),
            F.get_json_object("payload", "$.after.checkout_id").alias("checkout_id_after"),
            F.get_json_object("payload", "$.before.checkout_id").alias("checkout_id_before"),
            F.get_json_object("payload", "$.source.table").alias("source_table"),
        )
        .withColumn(key, F.coalesce("after_key", "before_key"))
        .withColumn("user_id", F.coalesce("user_id_after", "user_id_before"))
        .withColumn("checkout_id", F.coalesce("checkout_id_after", "checkout_id_before"))
    )
    invalid_reason = (
        F.when(F.get_json_object("raw_payload", "$").isNull(), F.lit("MALFORMED_JSON"))
        .when(~F.col("operation").isin("r", "c", "u", "d"), F.lit("INVALID_CDC_OPERATION"))
        .when(F.col(key).isNull() | (F.length(F.col(key)) == 0), F.lit("MISSING_CDC_PRIMARY_KEY"))
        .when(F.col("source_lsn").isNull(), F.lit("MISSING_SOURCE_LSN"))
    )
    invalid = (
        parsed.withColumn("reason_code", invalid_reason)
        .filter(F.col("reason_code").isNotNull())
    ...
    duplicates = duplicates.withColumn("reason_code", F.lit("DUPLICATE_CDC_EVENT")).withColumn(
        "reason_description", F.lit("CDC event already exists")
    )
    ...
    if table == "users_cdc_clean":
        ...
        accepted = accepted.withColumn(
            "order_id",
            F.coalesce(
                F.get_json_object("after_json", "$.order_id"),
                F.get_json_object("before_json", "$.order_id"),
            ),
        ).withColumn(
            "product_id",
            F.coalesce(
                F.get_json_object("after_json", "$.product_id"),
                F.get_json_object("before_json", "$.product_id"),
            ),
        )
...
def main() -> int:
    ...
    try:
        ...
        event_time = F.coalesce(
            ...
            F.to_timestamp(
                F.get_json_object("raw_payload", "$.timestamp"), "yyyy-MM-dd'T'HH:mm:ssX"
            ),
            F.col("kafka_timestamp"),
        )
        stream = kafka.withColumn("event_time", event_time).withWatermark(
            "event_time", f"{args.allowed_lateness_minutes} minutes"
        )
        ...
        query = (
            stream.writeStream.queryName("streaming-ingestion")
