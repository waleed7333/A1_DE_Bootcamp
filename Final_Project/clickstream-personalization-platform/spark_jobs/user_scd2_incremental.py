...
PROFILE_FIELDS = ("email", "first_name", "last_name", "membership_type", "account_status", "country_code", "city")
USER_SCHEMA = StructType([StructField("user_id", StringType(), True), *[StructField(name, StringType(), True) for name in PROFILE_FIELDS], StructField("created_at", StringType(), True), StructField("updated_at", StringType(), True)])
...
def latest_watermark(spark: SparkSession) -> int:
    ...
    rows = spark.table(WATERMARKS).filter((F.col("job_name") == "user_scd2") & (F.col("source_name") == "users_cdc_clean")).orderBy(F.col("updated_at").desc()).limit(1).collect()
...
def normalize_events(spark: SparkSession) -> DataFrame:
    ...
    return parsed.select(
        ...
        *[F.col(f"record.{field}").alias(field) for field in PROFILE_FIELDS],
        ...
        F.col("operation"), F.col("source_lsn").cast("long").alias("source_lsn"),
        F.col("source_ts_ms").cast("long").alias("source_ts_ms"),
        F.col("kafka_partition").cast("int").alias("kafka_partition"), F.col("kafka_offset").cast("long").alias("kafka_offset"),
        F.col("processed_at"),
...
def build_user_history(events: DataFrame) -> DataFrame:
    ...
    order = Window.partitionBy("user_id").orderBy(F.col("source_lsn").asc_nulls_last(), F.col("source_ts_ms").asc_nulls_last(), F.col("kafka_partition"), F.col("kafka_offset"))
    profile_hash = F.sha2(F.concat_ws("||", *[F.coalesce(F.col(name), F.lit("<NULL>")) for name in PROFILE_FIELDS]), 256)
    prepared = (
        events.withColumn("is_deleted", F.col("operation") == F.lit("d"))
        .withColumn("account_status", F.when(F.col("operation") == "d", F.lit("deleted")).otherwise(F.col("account_status")))
        .withColumn("profile_hash", F.concat_ws("|", profile_hash, F.col("is_deleted").cast("string")))
        ...
        .withColumn("safe_ts_ms", F.coalesce(F.col("source_ts_ms"), (F.unix_timestamp("processed_at") * F.lit(1000)).cast("long")))
        # Add version_sequence microseconds so same-millisecond changes keep valid ranges.
        .withColumn("effective_from", F.expr("timestamp_micros(safe_ts_ms * 1000 + version_sequence)"))
    )
    version_order = Window.partitionBy("user_id").orderBy("effective_from", "source_lsn", "kafka_partition", "kafka_offset")
    return prepared.withColumn("effective_to", F.lead("effective_from").over(version_order)).withColumn("is_current", F.col("effective_to").isNull().cast(BooleanType())).select(
        "user_id", *PROFILE_FIELDS, "is_deleted", "effective_from", "effective_to", "is_current", "version_sequence", "source_lsn", "created_at", "updated_at", F.current_timestamp().alias("processed_at")
    )
...
def validate_scd2(df: DataFrame) -> dict[str, int]:
    ...
    duplicate_current = df.filter("is_current = true").groupBy("user_id").count().filter("count != 1").count()
    invalid_ranges = df.filter("is_current = false").filter(F.col("effective_to").isNull() | (F.col("effective_to") <= F.col("effective_from"))).count()
    if users == 0 or current != users or duplicate_current or invalid_ranges:
        raise RuntimeError(f"Invalid SCD2 result: users={users}, current={current}, duplicate_current={duplicate_current}, invalid_ranges={invalid_ranges}")
    return {"users": users, "current_rows": current, "duplicate_current": duplicate_current, "invalid_ranges": invalid_ranges}


def record_run(spark: SparkSession, run_id: str, status: str, started: datetime, *, input_count: int, output_count: int, error: str | None = None) -> None:
    ...
    spark.createDataFrame([(run_id, "user_scd2", "users_cdc_clean", status, started, now_utc(), input_count, output_count, 0, 0, output_count, error, now_utc())], schema).writeTo(PIPELINE_RUNS).append()
...
def main() -> int:
    ...
    try:
        ...
        spark.sql(f"CREATE OR REPLACE TABLE {TARGET} USING iceberg AS SELECT * FROM next_user_profile_scd2")
        max_lsn = new_events.agg(F.max("source_lsn").alias("max_lsn")).collect()[0]["max_lsn"]
        spark.createDataFrame([("user_scd2", "users_cdc_clean", str(max_lsn), int(max_lsn or 0), None, now_utc())], "job_name string, source_name string, last_processed_id string, last_processed_offset long, last_snapshot_id string, updated_at timestamp").writeTo(WATERMARKS).append()
        record_run(spark, args.run_id, "PASSED", started, input_count=input_count, output_count=output_count)
        print(json.dumps({"status": "PASSED", "run_id": args.run_id, "incremental_events": input_count, "rows": output_count, **metrics}))
        return 0
    except Exception as error:
        if spark is not None:
            try:
                record_run(spark, args.run_id, "FAILED", started, input_count=input_count, output_count=output_count, error=f"{type(error).__name__}: {error}")
            except Exception:
                pass
