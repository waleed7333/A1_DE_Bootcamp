...
AUDIT = ("pipeline_runs", "quality_metrics", "quarantine_records", "external_api_failures", "watermarks", "validation_runs", "serving_builds")
RAW = ("kafka_messages",)
...
def write_report(run_id: str, payload: dict) -> None:
    ...
    (PROJECT_RUNTIME / f"verify_lakehouse_bootstrap_{run_id}.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    try:
        ...
        invalid_count = catalog.filter(F.col("price").isNull() | (F.col("price") < 0) | F.col("inventory").isNull() | (F.col("inventory") < 0)).count()
        if row_count != expected or duplicate_count or invalid_count:
            raise RuntimeError(f"Product catalog failed: rows={row_count}/{expected}, duplicates={duplicate_count}, invalid={invalid_count}")
        payload = {"status": "PASSED", "run_id": options.run_id, "finished_at_utc": datetime.now(UTC).isoformat(), "table_count": len(RAW) + len(PROCESSED) + len(AUDIT), "product_catalog_rows": row_count}
        write_report(options.run_id, payload)
        print(json.dumps(payload))
        return 0
    except Exception as error:
        payload = {"status": "FAILED", "run_id": options.run_id, "finished_at_utc": datetime.now(UTC).isoformat(), "error": f"{type(error).__name__}: {error}"}
        write_report(options.run_id, payload)
